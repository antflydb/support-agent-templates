...
| --- | --- | --- | --- | --- | --- | --- |
| **Knowledge Support — Next.js/Vercel** | Product template combining admin, ingestion, hosted support, search, and `@antfly/components` embedding | Server-side Antfly and generation credentials; browser talks only to application-owned chat or session routes | Reuses the same read-only retrieval contract while adding source sync, configurable assistants, conventional search, analytics, and escalation | Warm MCP reuse for requests; crawls and repository synchronization run as resumable jobs outside a single serverless request | Reusable `SupportProvider` and `SupportChat` components plus hosted pages and an admin workflow | Customers building their own Antfly-backed knowledge base and Kapa-like support product |
...
| Need | Start with |
| --- | --- |
| Complete knowledge base, docs search, and embedded support product | Knowledge Support — Next.js/Vercel |
