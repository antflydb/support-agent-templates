...
| **Codex** | Native remote MCP configuration plus durable `AGENTS.md` instructions | Codex supports a URL and `bearer_token_env_var`, so the tracked config references the secret by environment-variable name | Native Antfly tools are filtered by approval policy and instructions; the model follows the shared raw QueryRequest contract | Codex owns MCP initialization and session handling; users can persist trust for the read-only tools | Project instructions separate documentation retrieval from the broader coding toolset and keep raw paths or credentials out of answers | Engineering and support work inside Codex projects |
