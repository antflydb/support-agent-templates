...
For a substantive question, run one Antfly `query` against `antfly_docs` in raw QueryRequest mode. Keep `tableName` outside `queryRequest`. Combine exact-term full-text search on `text`, semantic search through `document_vectors`, RRF merge, chunk-level hierarchy output including source and unit, and limit 6. Do not issue parallel Antfly calls. If the first result contains sufficient explanatory chunk text, answer immediately. Use at most one focused fallback query when the first result is empty or lacks evidence required by the question. Never make more than two queries per answer.
